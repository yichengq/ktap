...
size_t csym_size(csymbol *cs)
{
	ffi_type type = csym_type(cs);
	switch(type) {
	case FFI_STRUCT:
		if (csym_struct(cs)->align == 0)
			init_csym_struct(csym_struct(cs));
...
size_t csym_struct_offset(csymbol_struct *csst, int idx)
{
	...
	if (idx < 0 || idx > nr)
		return -1;
	for (i = 0; i < idx; i++) {
		csymbol *var_cs = csymst_mb(csst, i);
		size_t var_size = csym_size(var_cs);
		size_t var_align = csym_align(var_cs);
