...
csymbol *ffi_get_csym_by_id(int id)
{
	return &cs_arr[id];
}
...
void cp_push_ctype_with_name(struct cp_ctype *ct, const char *name, int nlen)
{
	...
	if (ct->pointers) {
		...
		if (i == cte_nr) {
			...
			cp_ctype_reg_type(csym_name(id_to_csym(ct->ffi_cs_id)),
						ct);
		} else {
...
int cp_symbol_build_pointer(struct cp_ctype *ct)
{
	...
	/* TODO: Check multi-level pointer is correct 24.11.2013(unihorn) */
...
int cp_ctype_reg_type(char *name, struct cp_ctype *ct)
{
...
void ctype_reg_builtin_types()
{
	struct cp_ctype ct;
	init_builtin_type(&ct, FFI_VOID);
	cp_ctype_reg_type("void", &ct);
	/* make void * a builtin type as well */
	ct.pointers++;
	ct.ffi_cs_id = cp_symbol_build_pointer(&ct);
	cp_ctype_reg_type("void *", &ct);

	init_builtin_type(&ct, FFI_INT8);
	cp_ctype_reg_type("int8_t", &ct);

	init_builtin_type(&ct, FFI_UINT8);
	cp_ctype_reg_type("uint8_t", &ct);

	init_builtin_type(&ct, FFI_INT16);
	cp_ctype_reg_type("int16_t", &ct);

	init_builtin_type(&ct, FFI_UINT16);
	cp_ctype_reg_type("uint16_t", &ct);

	init_builtin_type(&ct, FFI_INT32);
	cp_ctype_reg_type("int32_t", &ct);

	init_builtin_type(&ct, FFI_UINT32);
	cp_ctype_reg_type("uint32_t", &ct);

	init_builtin_type(&ct, FFI_INT64);
	cp_ctype_reg_type("int64_t", &ct);

	init_builtin_type(&ct, FFI_UINT64);
	cp_ctype_reg_type("uint64_t", &ct);
}
